...
// serverCommand start a app serve
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if cronDaemon {
			...
			cntxt := &daemon.Context{
				...
				Umask: 027,
				//子进程的参数，按照这个参数设置，子进程命令为 ./hade cron start --daemon=true
				Args: []string{"", "cron", "start", "--daemon=true"},
			}
			//启动子进程，d不为空表示为当前是父进程，d为空表示当前是子进程
			d, err := cntxt.Reborn()
			if err != nil {
				return err
			}
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if content != nil && len(content) > 0 {
			...
			if util.CheckProcessExist(pid) {
				//window 杀死进程 todo 如何兼容linux
				cmd := exec.Command("taskkill", "/pid", cast.ToString(pid), "-f")
				//执行
				err = cmd.Run()
				if err != nil {
					return err
